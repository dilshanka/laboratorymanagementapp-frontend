...
const Dashboard = () => {
  ...
  const [formDetails, setFormDetails] = useState({
    fullName: location.state?.patient.fullName,
    address: location.state?.patient.address,
    contactNumber: location.state?.patient.contactNumber,
    date: location.state?.patient.date,
    gender: location.state?.patient.gender,
    nameOfLabTest: location.state?.patient.nameOfLabTest,
  ...
  const handleUpdateClick = () => {
    // Make a PUT request to the backend API endpoint with formDetails
    fetch(
      `http://localhost:8080/api/v1/patient/${location.state?.patient._id}`,
      {
        method: 'PUT',
        headers: {
